...
# are you with the same partner as in previous wave?
data_subset$an225 <- 1

# Delete the labels created by SPSS
data_subset <- haven::zap_labels(data_subset)

# Data cleanup

dplyr::rename(.data = data_subset,
              'nkids_t1' = 'ankids',
              'nkdis_t2' = 'bnkids',
              'nkdis_t3' = 'cnkids')
...
data_subset <-
    dplyr::rename_with(.data = data_subset,
                       .fn = ~ stringr::str_replace(.x,
                                                    pattern = 'c721([:lowercase:])',
                                                    replacement = 'c721_\\1'),
                       .cols = dplyr::matches('c721'))

# equal to lines 46-51
# data_subset <-
#     dplyr::rename_with(.data = data_subset,
...
select(data_subset, dplyr::matches('c721'))
